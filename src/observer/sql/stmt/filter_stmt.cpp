    ...
    const ConditionSqlNode &condition, FilterUnit *&filter_unit)
{
  ...
  //  const auto right_attr_name =
  //  default_table->table_meta().field(condition.right_attr.attribute_name.c_str())->type();
  ...
  } else if (condition.right_attr.attribute_name == "u_date" && filter_unit->left().value.attr_type() == CHARS &&
             filter_unit->right().value.attr_type() == DATES) {
    int64_t date;
    ...
    ComparisonExpr *condition, FilterUnit *&filter_unit)
{
  ...
  LOG_DEBUG("filter_unit->left().value.data(): %s, filter_unit->right().value.data(): %s", filter_unit->left().value.data(), filter_unit->right().value.data());
  if (condition->left()->type() == ExprType::REL_ATTR &&
      static_cast<RelAttrExprSqlNode *>(condition->left().get())->get_rel_attr()->attribute_name == "u_date" &&
  ...
  } else if (condition->right()->type() == ExprType::REL_ATTR &&
             static_cast<RelAttrExprSqlNode *>(condition->right().get())->get_rel_attr()->attribute_name == "u_date" &&
             filter_unit->left().value.attr_type() == CHARS && filter_unit->right().value.attr_type() == DATES) {
    int64_t date;
    bool    valid = serialize_date(&date, filter_unit->left().value.data());
    if (!valid) {
      LOG_DEBUG("invalid date: %s", filter_unit->left().value.data());
