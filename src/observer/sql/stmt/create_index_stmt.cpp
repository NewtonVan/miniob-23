...
std::string CreateIndexStmt::to_string(const std::vector<std::string>& strings) {
  std::string result;
...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) || create_index.attribute_names.size() == 0) {
    LOG_WARN("invalid argument. db=%p, table_name=%p, index name=%s, attribute name=%s",
        db, table_name, create_index.index_name.c_str(), to_string(create_index.attribute_names).c_str());
  ...
  for (int i = 0; i < create_index.attribute_names.size(); ++i) {
    ...
    FieldMeta *mutable_field_meta = const_cast<FieldMeta *>(field_meta); // 创建非常量指针
    field_metas.push_back(mutable_field_meta);
  }
